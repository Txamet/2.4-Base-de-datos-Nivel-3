...
db.getCollection('restaurantes').find( {}, { _id: 0, restaurant_id: 1, name: 1, borough: 1, cuisine: 1 } );
...
db.getCollection('restaurantes').find( {}, { _id: 0, restaurant_id: 1, name: 1, borough: 1, 'address.zipcode': 1 } );
...
db.getCollection('restaurantes') .find({ borough: 'Bronx' }) .limit(5) .skip(5);
...
db.getCollection('restaurantes').find({ grades: { $elemMatch: { score: { $gt: 80, $lt: 100 } } } });
...
db.getCollection('restaurantes').find({ $and: [ { cuisine: { $ne: 'American' } }, { 'grades.score': { $gt: 70 } }, { 'address.coord.0': { $lt: -65.754168 } } ] });
...
db.getCollection('restaurantes').find({ cuisine: { $ne: 'American' }, 'grades.score': { $gt: 70 }, 'address.coord.0': { $lt: -65.754168 } });
...
db.getCollection('restaurantes') .find({ cuisine: { $ne: 'American' }, 'grades.grade': 'A', borough: { $ne: 'Brooklyn' } }) .sort({ cuisine: -1 });
...
db.getCollection('restaurantes').find( { name: RegExp('^Wil') }, { restaurant_id: 1, borough: 1, cuisine: 1, name: 1 } );
...
db.getCollection('restaurantes').find( { name: RegExp('ces$') }, { restaurant_id: 1, borough: 1, cuisine: 1, name: 1 } );
...
db.getCollection('restaurantes').find( { name: RegExp('Reg') }, { restaurant_id: 1, borough: 1, cuisine: 1, name: 1 } );
// 17. Escriu una consulta per trobar els restaurants que pertanyen al Bronx i preparen plats Americans o xinesos.
db.getCollection('restaurantes').find({ borough: 'Bronx', $or: [ { cuisine: 'American' }, { cuisine: 'Chinese' } ] });
...
db.getCollection('restaurantes').find( { 'grades.score': { $lt: 10 } }, { restaurant_id: 1, name: 1, borough: 1, cuisine: 1, 'grades.score': 1 } );
...
db.getCollection('restaurantes').find( { $and: [ { cuisine: 'Seafood' }, { $nor: [ { cuisine: ['American', 'Chinese'] }, { name: RegExp('^Will') } ] } ] }, { restaurant_id: 1, name: 1, borough: 1, cuisine: 1 } );
...
db.getCollection('restaurantes').find( { grades: { $elemMatch: { grade: 'A', score: 11, date: ISODate('2014-08-11T00:00:00.000Z') } } }, { restaurant_id: 1, name: 1, grades: 1 } );
...
db.getCollection('restaurantes').find( { 'grades.1.grade': 'A', 'grades.1.score': 9, 'grades.1.date': ISODate( '2014-08-11T00:00:00.000Z' ) }, { restaurant_id: 1, name: 1, grades: 1 } );
...
db.getCollection('restaurantes').find( { 'address.coord.1': { $gt: 42, $lt: 52 } }, { restaurant_id: 1, name: 1, address: 1 } );
...
db.getCollection('restaurantes').find({ 'address.street': { $eq: null } });
...
db.getCollection('restaurantes').find( { 'grades.score': { $mod: [7, 0] } }, { restaurant_id: 1, name: 1, grades: 1 } );
...
db.getCollection('restaurantes').find( { name: RegExp('mon.*', 'i') }, { name: 1, borough: 1, 'address.coord': 1, cuisine: 1 } );
...
db.getCollection('restaurantes').find( { name: RegExp('^Mad') }, { name: 1, borough: 1, 'address.coord': 1, cuisine: 1 } );
